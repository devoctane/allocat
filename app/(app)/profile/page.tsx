  ...
  return (
    <div className="pt-12 px-6 max-w-2xl mx-auto space-y-12 pb-32">
      ...
      <header className="space-y-2 py-8">
        ...
        <h1 className="text-[3.5rem] font-black tracking-tighter leading-none uppercase text-foreground">Profile</h1>
        <p className="text-muted-foreground font-medium tracking-tight">The architecture of your financial identity.</p>
      </header>

      {/* User Profile Section (Asymmetric Layout) */}
      <section className="grid grid-cols-1 md:grid-cols-3 gap-8 items-center bg-card/50 p-8 rounded-lg border border-border">
        ...
        <div className="col-span-2 space-y-1">
          <p className="text-[0.6875rem] uppercase tracking-[0.2em] text-muted-foreground font-bold">Verified Account</p>
          <h2 className="text-3xl font-extrabold tracking-tight text-foreground">{profile?.full_name || 'Anonymous Architect'}</h2>
          <p className="text-muted-foreground font-mono tracking-tight text-sm">{profile?.email || user.email}</p>
      ...
      {/* Account Settings Section (Tonal Layering) */}
      <section className="space-y-6">
        ...
        <div className="space-y-1">
          <button className="w-full flex justify-between items-center p-5 bg-card/50 border border-border hover:bg-muted transition-colors group">
            ...
            <MaterialSymbol icon="chevron_right" className="text-muted-foreground group-hover:translate-x-1 transition-transform" />
          </button>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-1 bg-border rounded-lg overflow-hidden border border-border">
          ...
          <div className="bg-card p-6 flex flex-col justify-between h-40 col-span-1">
            ...
            <div>
              ...
              <p className="text-xs text-muted-foreground uppercase tracking-widest mt-1">United States Dollar</p>
            </div>
          </div>
          ...
          <div className="bg-card p-6 flex flex-col justify-between h-40 col-span-1">
            <div className="flex justify-between items-start">
              ...
              <span className="text-[10px] bg-muted text-foreground px-2 py-1 uppercase font-bold tracking-tighter">Encrypted</span>
            </div>
            <div className="flex justify-between items-end">
              <div>
                <p className="font-bold text-foreground">Data Privacy</p>
                <p className="text-xs text-muted-foreground uppercase tracking-widest mt-1">Manage shared data and analytics</p>
              </div>
      ...
      <section className="py-12 flex flex-col items-center gap-8">
        <form action={signOut} className="w-full max-w-sm">
          <button type="submit" className="group relative flex items-center justify-center w-full py-5 border border-red-500/30 hover:bg-red-500/10 transition-all duration-300 active:scale-95">
